use crate::{
  cmd::Cmd, hash128_bytes, kad::Kad, kad_net::kad_net, key, key::hash128_bytes, pool::spawn,
  typedef::ToAddr, util::udp::send_to, var,
};
...
const VERSION: &[u8] = &var::VERSION.to_le_bytes();
  ...
  pub fn recv(&self, msg: &[u8], src: Addr) -> Result<()> {
    ...
    if msg_len == 0 {
      if self.ping.has(addr) {
        ...
        // 1 + 2 + 30  = 33
        reply!(Cmd::Ping, VERSION, self.pk())
      }
